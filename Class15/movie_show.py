...
class MovieShow:
    ...
    def book_tickets(self, customer, quantity):
        if quantity <= 0 or quantity > MAX_TICKETS_PER_BOOKING:
            raise InvalidBookingError(
                f"Invalid quantity. You can book between 1 and {MAX_TICKETS_PER_BOOKING} tickets."
            )
    ...
    def display_info(self):
        print(
            f"Title: {self.__title} | Capacity: {self.__capacity} | Booked Seats: {self.__booked_seats} | Status: {self.__status.value}"
        )

    # Bonus A
    def reopen_show(self):
        if self.__status == ShowStatus.CANCELLED:
            raise ShowCancelledError("Cancelled shows cannot be reopened.")

        if self.__status != ShowStatus.SOLD_OUT:
            raise InvalidStatusError("Only sold-out shows can be reopened.")
...
show = MovieShow("Avengers", 100, 0, ShowStatus.OPEN)
...
try:
    sold_out_show = MovieShow("Batman", 10, 10, ShowStatus.SOLD_OUT)
...
try:
    cancelled_show = MovieShow("Superman", 50, 0, ShowStatus.CANCELLED)
